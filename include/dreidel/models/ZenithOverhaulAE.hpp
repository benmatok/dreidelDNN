...
public:
    ZenithCompressBlock(size_t in_channels, const std::string& init_scheme = "he")
        : ZenithCompressBlock(in_channels, in_channels * 2, init_scheme) {}

    ZenithCompressBlock(size_t in_channels, size_t out_channels, const std::string& init_scheme = "he") {
...
public:
    ZenithExpandBlock(size_t in_channels, const std::string& init_scheme = "he")
        : ZenithExpandBlock(in_channels, in_channels / 2, init_scheme) {}

    ZenithExpandBlock(size_t in_channels, size_t out_channels, const std::string& init_scheme = "he") {
        // Reverse of Compress
        // To match Compress(in -> out) which uses unshuffle(2), we need shuffle(2).
        // Compress: [C, H, W] -> Unshuf -> [4C, H/2, W/2] -> ... -> [Out, H/2, W/2]
        // Expand:   [In, H, W] -> ... -> [4*Out, H, W] -> Shuffle -> [Out, 2H, 2W]

        // We need the input to the shuffler to be out_channels * 4.
        size_t expanded_dim = out_channels * 4;
        ...
        // 3. Local Fidelity
        // We want the group count to match the final output channels so that each channel in output
        // comes from a group of 4 in expanded space (spatial pixels).
        // expanded_dim = 4 * out_channels.
        // groups = out_channels.
        pixel_mix_ = std::make_unique<layers::Conv2D<T>>(expanded_dim, expanded_dim, 3, 1, 1, out_channels);
        norm_ = std::make_unique<layers::GroupNorm<T>>(32, expanded_dim);
        act_ = std::make_unique<layers::GELU<T>>();
...
public:
    ZenithOverhaulAE(const std::string& init_scheme = "he") {
        ...
        // Enc5: 512 -> 512 (Capped)
        layers_.push_back(std::make_unique<ZenithCompressBlock<T>>(512, 512, init_scheme));
        // Enc6: 512 -> 512 (Capped)
        layers_.push_back(std::make_unique<ZenithCompressBlock<T>>(512, 512, init_scheme));
        ...
        // Decoder (Inverted)
        // Dec7: 512 -> 512
        layers_.push_back(std::make_unique<ZenithExpandBlock<T>>(512, 512, init_scheme));
        // Dec6: 512 -> 512
        layers_.push_back(std::make_unique<ZenithExpandBlock<T>>(512, 512, init_scheme));
        // Dec5: 512 -> 512
        layers_.push_back(std::make_unique<ZenithExpandBlock<T>>(512, 512, init_scheme));
        // Dec4: 512 -> 256 (Start scaling down)
        layers_.push_back(std::make_unique<ZenithExpandBlock<T>>(512, 256, init_scheme));
        // Dec3: 256 -> 128
