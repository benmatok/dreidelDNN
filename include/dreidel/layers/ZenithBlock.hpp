...
public:
    ...
    {
        // Init
        T stddev = std::sqrt(static_cast<T>(2.0) / (kernel_size * kernel_size * channels));
    ...
    Tensor<T> backward(const Tensor<T>& grad_output) override {
        ...
        #pragma omp parallel
        {
            ...
            std::vector<T> buf(C), buf_spectral(C), buf_scaled(C), dL_dPreAct(C), dL_dPerm(C), dL_dScaled(C), dL_dSpectral(C), dL_dEyes(C);
